...
export function getStudentBillingCycle(student, feePayments = [], asOfDate = new Date()) {
  ...
  return {
    ...
    cycleStartDate,
    cycleStartFormatted: format(cycleStartDate, 'dd MMM yyyy'),
    nextDueDate,
    nextDueDateFormatted: format(nextDueDate, 'dd MMM yyyy'),
    cycleEndFormatted: format(new Date(nextDueDate.getTime() - 24 * 60 * 60 * 1000), 'dd MMM yyyy'),
    totalMonthsPassed,
